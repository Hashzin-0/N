...
export default function PortaisConfiaveisSection({
  ...
  return (
    <section id="pesquisador_portais" className="space-y-6">
        ...
        <div className="flex items-center gap-3 border-b border-[#F0EDE5] dark:border-[#2C3328] pb-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="hidden sm:inline-block text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full bg-[#D4A373]/20 text-[#8D6E63] dark:text-[#D4A373]">
                Deck 3D • Arraste para Alternar
              </span>
            </div>
      ...
      {/* PORTALS 3D CARD SWAP (SEM SETAS < E >, SWAP ESQUERDA E DIREITA) */}
      <div className="w-full max-w-2xl mx-auto py-4 px-2 sm:px-4">
        <CardSwap
          className="h-[490px] sm:h-[460px] w-full"
          offsetX={16}
          offsetY={18}
          scaleStep={0.045}
        >
          {RELIABLE_PORTALS.map((portal) => {
            ...
            return (
              <div
                key={portal.id}
                className="h-full w-full bg-white dark:bg-[#1A1E17] p-6 sm:p-7 rounded-3xl border border-[#E5E2D9] dark:border-[#2C3328] shadow-xl flex flex-col justify-between space-y-4 select-none"
              ...
              >
                <div className="space-y-3">
                  ...
                  {/* HIGHLIGHTED JOURNALS - COM FUNDO OPACO SÓLIDO */}
                  {portal.highlightedJournals && portal.highlightedJournals.length > 0 && (
                    <div className="bg-[#FAF8F5] dark:bg-[#121511] p-3 rounded-2xl border border-[#F0EDE5] dark:border-[#242A20] space-y-1.5 shadow-xs">
                      <span className="text-[11px] font-semibold text-[#2E6F40] dark:text-[#9CB386] flex items-center gap-1.5">
                        <BookmarkCheck className="h-3.5 w-3.5 text-[#2E6F40] dark:text-[#9CB386] shrink-0" /> Principais periódicos & séries:
                      </span>
                      <ul className="text-[11px] text-[#5A5A40] dark:text-[#C5D9B0] space-y-1 list-disc list-inside font-medium">
                        {portal.highlightedJournals.slice(0, 3).map((journal, i) => (
                ...
                {/* ACTION BUTTONS */}
                <div className="pt-3 border-t border-[#F0EDE5] dark:border-[#2C3328] flex flex-col sm:flex-row items-center gap-2 relative z-10">
                  <a
